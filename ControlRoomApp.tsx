...
const STATUS_COLORS = ["#73b7ff", "#45d8ff", "#a7b8ff", "#c7a7ff", "#ff9fc5", "#ffc46b", "#79e6d0"];
...
function BrainView({
  ...
  const priorityIdea = useMemo(() => {
    return [...ideas].sort((left, right) => commercialScore(right, statuses) - commercialScore(left, statuses))[0] || null;
  }, [ideas, statuses]);
  const saleReadyCount = ideas.filter((idea) => idea.status === "En venta").length;
  const executionCount = ideas.filter((idea) => ["En desarrollo", "En test", "En producción"].includes(idea.status)).length;
  const highValueCount = ideas.filter((idea) => idea.value === "Alto").length;
  ...
  return (
    <div className={`brainWrap ${brainMode === "flat" ? "flatMode" : ""}`}>
      ...
      <section className="brainCommandPanel" aria-label="Resumen comercial del cerebro">
        <div className="brainCommandCopy">
          <span>Motor idea a venta</span>
          <strong>{priorityIdea?.name || "Sin prioridad todavía"}</strong>
          <p>{priorityIdea ? nextCommercialAction(priorityIdea, statuses) : "Carga ideas para activar el ranking comercial."}</p>
        </div>
        <div className="brainCommandStats">
          <span><b>{saleReadyCount}</b> en venta</span>
          <span><b>{executionCount}</b> en ejecución</span>
          <span><b>{highValueCount}</b> alto valor</span>
        </div>
        <button className="btn brainCta" type="button" disabled={!priorityIdea} onClick={() => priorityIdea && onOpen(priorityIdea)}>
...
function commercialScore(idea: Idea, statuses: Status[]) {
  const progress = developmentPercent(idea, statuses);
  const valueScore = idea.value === "Alto" ? 34 : idea.value === "Medio" ? 18 : 6;
  const effortPenalty = idea.effort === "Alta" ? 10 : idea.effort === "Media" ? 4 : 0;
  const saleBonus = idea.status === "En venta" ? 22 : idea.status === "En producción" ? 16 : 0;
  return progress + valueScore + saleBonus - effortPenalty;
}

function nextCommercialAction(idea: Idea, statuses: Status[]) {
  if (idea.status === "En venta") return "Preparar oferta, precio, caso de uso y lista de prospectos para cerrar primeras ventas.";
  if (idea.status === "En producción") return "Convertir la versión publicada en demo comercial con CTA, prueba guiada y objeciones resueltas.";
  if (idea.status === "En test") return "Cerrar validación, documentar resultados y convertir aprendizajes en argumento de venta.";
  if (idea.status === "En desarrollo") return "Reducir alcance a un flujo demo que enseñe valor en menos de dos minutos.";
  if (developmentPercent(idea, statuses) >= 30) return "Completar investigación, cliente objetivo, pricing inicial y prueba de demanda.";
  return "Aterrizar el problema, comprador, promesa y primer entregable antes de construir.";
}
